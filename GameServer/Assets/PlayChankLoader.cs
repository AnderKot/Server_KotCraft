...
public class PlayerChankLoader : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        RenderThread.Start();
        //RenderNearChanks(Vector3Int.zero);
    }
    ...
    private void RenderNearChanks(Vector3Int newCurrChank)
    {
        ...
        if (Delta.x == 0)
        {
            RenderList.Add(newCurrChank + Delta + (Vector3Int.right * 20));
            RenderList.Add(newCurrChank + Delta + (Vector3Int.left * 20));
        }
        else
        {
            RenderList.Add(newCurrChank + Delta + (Vector3Int.forward * 20));
            RenderList.Add(newCurrChank + Delta + (Vector3Int.back * 20));
        }
